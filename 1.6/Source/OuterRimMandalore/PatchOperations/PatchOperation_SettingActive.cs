...
namespace OuterRimMandalore
{
    public class PatchOperation_SettingActive : PatchOperation
    {
        ...
        public override bool ApplyWorker(XmlDocument xml)
        {
            ...
            ModContentPack modContentPack = OuterRimMandaloreMod.mod.Content;
            for (int i = 0; i < settings.Count(); i++)
            {
                if (!OuterRimMandaloreMod.settings.IsValidSetting(settings[i]))
                {
                ...
                if (OuterRimMandaloreMod.settings.GetEnabledSettings.Contains(settings[i]))
                {
                    flag = true;
                    break;
                }
